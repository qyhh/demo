...
//得到查询列表
// if(!city) {
//   return errorMsg('请输入城市!')
// } else if(!post) {
//   return errorMsg('请输入职位!')
// }
export const getSelectList =(type,city,post)=>{

  return async dispatch => {
    const response = await reqSelectList(type,city,post)
    const result = response.data
    // 得到结果后, 分发一个同步action
    if(result.code===0) {
      dispatch(receiveSelectList(result.data))
    }
  }
}
